...
resource "azurerm_lb_nat_rule" "this" {
  ...
  name                                        = format("%s_%s", element(var.lb_nat_remote_port[element(keys(var.lb_nat_remote_port), count.index)], 0), count.index)
  protocol                                    = element(var.lb_nat_remote_port[element(keys(var.lb_nat_remote_port), count.index)], 0)
  resource_group_name                = var.resource_group_name
  frontend_port_start                    = element(var.lb_nat_remote_port[element(keys(var.lb_nat_remote_port), count.index)], 2)
  frontend_port_end                      = element(var.lb_nat_remote_port[element(keys(var.lb_nat_remote_port), count.index)], 3)
}
